...
public class MongoDb_Recuperacion {
	...
	private static void JugadoresCollection() {
		...
		MongoDatabase db = m.getDatabase("IE_Recu"); // Seleccion de la Database, la cual si no se encuentra prehecha la crea.
	...
	private static void JugadoresJSONDatos() {
		try {
			FileReader fileReader = new FileReader("src\\main\\java\\RECUPERACION\\Recursos\\Jugadores.json"); //Especificamos la ruta del archivo JSON.
			...
			String jsontxt;
			while ((jsontxt = bufferedReader.readLine())!= null) {
				System.out.println(jsontxt);
				Document doc = new Document(org.bson.Document.parse(jsontxt));
				doc.put("Nombre", jsontxt);
				doc.put("Posicion", jsontxt);				
				doc.put("Equipo", jsontxt);
